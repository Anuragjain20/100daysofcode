...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(math.floor(reps / 2)):
            marks += "✔ "
        checkmarks_lb.config(text=marks)
...
# Timer label
timer_lb = Label(text="Timer", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 50))
...
# Checkmarks
checkmarks_lb = Label(bg=YELLOW, fg=GREEN)
checkmarks_lb.grid(row=4, column=1)
